...
// 			$config = array_merge ( ( array ) $config, ( array ) $_POST ['config'] );
	...
	function index() {
		...
		if (file_exists ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/pigcms/Index_' . $this->config ['template_index'] . '.html' )) {
			...
			//My Test
			$this->assign ( 'wx_time_stamp', NOW_TIME );
...
//			获取用户openId、uid和用户信息  eta\trunk\Application\Common\Common\function.php
//			$this->assign ( 'openid', $openid = get_openid () );
//			$this->assign("uid", $uid = get_mid());
//			$this->assign('userinfo',$userinfo = getWeixinUserInfo($openid));
